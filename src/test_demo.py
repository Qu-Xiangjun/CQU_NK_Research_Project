...
# #             current_image = Image.fromarray(graph)
# #             imgtk = ImageTk.PhotoImage(image=current_image)
# #             panel_lidar.config(image=imgtk)
# #             # window.obr = imgtk
# #             panel_lidar2.config(image=imgtk)
...
#     if success:
#         cv2image = cv2.cvtColor(img, cv2.COLOR_BGR2RGBA)#转换颜色从BGR到RGBAcv2.VideoCapture(0,cv2.CAP_DSHOW).read()
#         print(type(cv2image))
...
#         comp_image_data = pickle.dumps(imgtk, protocol=0)  # 序列化为字节序列
#         panel.imgtk = imgtk
